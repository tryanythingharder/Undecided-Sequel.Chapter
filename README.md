...
- **一键成漫**：对话进行到任意回合，画廊点「生成漫画回放」——引擎从全量剧情账本精选关键幕（AI 分镜师取舍，
  8~32 幕可调）或按回合逐幕展开，自动生成**角色外观表**（发色/服装/体型）逐幕注入，跨幕不变脸；
- **起止自选**：默认从第 1 回合讲到当前进度，也可自选起止回合；继续推进剧情后再点一键，**只补画新幕**（已有的幕保留）；
- **断点续跑**：逐幕串行绘制，随时暂停/继续；中断重启后自动差量补缺；
- **阅读与分享**：应用内漫画阅读视图（←→ 翻页、单幕重绘），一键导出**自包含 HTML 漫画**（内嵌图片，浏览器直接打开）。

### 角色闪卡工坊

- **选角制卡**：画廊一键「生成角色闪卡」——从世界线选角色，AI 规划卡面（两段式生图 + 自动抠图排版）；
