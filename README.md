...
- 工作区级绑定：不同世界线可固定不同内核；换内核需显式迁移（引擎按 sha1 校验内核版本，防串线）；
- 自定义内核：设置 → 选择内核文件，指到任意 Markdown 即可；内核可按 `KERNEL_META` 声明标题、
  开场白与出身预设，未声明则使用通用模板。
...
| 文件 | 说明 |
|---|---|
| `SixWorlds-Setup-1.3.0.exe` | 安装版：可选目录、创建快捷方式 |
| `SixWorlds-Portable-1.3.0.exe` | 免安装单文件，双击即用 |
...
├── engine/               # 故事状态引擎（内核无关）：九大账本 / 检索 / Context / 校验 / 提交 / 快照 / Pending
├── renderer/
│   ├── index.html        # 主界面 DOM
