...
- 世界之灵桌宠：`test-bloub.cjs`（69 项：15 状态 / 3000 帧确定性 / 规则问答契约 / 回复清洗管线 / 大脑路由）、`test-bloub-e2e.cjs`（真实渲染 42 项：边距常驻 / 视线真实位置差 / 右键快捷菜单 / 气泡问答 / 拖拽记忆 / 事件反应 / 窄窗适配 / 本地小模型一键接入全流程 / 云端→本地大脑路由切换——假下载源接缝，不在 CI 下 400MB）；
