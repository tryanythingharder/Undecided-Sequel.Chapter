...
（九大账本长期记忆 + 结构化落账），支持世界线分歧回溯（IF 线）、AI 插图生成、**一键漫画回放**
（把整条世界线从头讲成分镜漫画）、**角色闪卡工坊**（把角色制成 3D 全息收藏卡）、画廊集与故事存档导出。
...
- **Context 硬预算**：发送给模型的上下文有硬上限与优先级（当前状态 > 场景 > 实体 > 决定 > 承诺 > 事实 > 事件），
  不随历史增长无限膨胀；
- **落账容错链**：回复先入列立即可读（阅读永不被记账阻塞），记账在后台进行——补录期间消息挂
  「记忆记账中」实时徽标，完成自动消失；模型没按协议输出状态块时，自动重试（带拒绝原因引导修正）→
  仍失败才挂起待补录（Pending），重启不丢、一键补录（补录同样携带首次拒绝原因）；
  线程/承诺状态的同义偏差（如把 threads 写成 ACTIVE）自动归一落账而非拒收整回合；
  协议标记的常见变体（全角括号/少箭头/两箭头/围栏包裹/裸 JSON/函数调用包装）均可自动识别；
  叙事尾部混入的协议残渣（围栏行/「状态块：」引导语/函数标头）会被自动剥净，绝不进正文显示；
  缺 turn_summary 但含其他协议键的尾部 JSON 也能兜底落账，叙事中段的示例 JSON 不会被误收；
...
- **整页生图**（顶栏「作品」→ 漫画回放，或 Ctrl+Shift+M）：**一页脚本只调用一次生图，返回一张完整漫画页**。
  分格、画面融合、斜边、气泡和中文对白都由生图模型一起绘制；阅读器不再把独立画格拼在一起，也不叠加文字。
  页脚本包含多段连续画面、角色外观、台词和整页构图意图，格数与大小随剧情变化，不固定模板。
  中文字形与跨格人物一致性取决于所选模型，不能保证无错字；旧版逐格存档仍按原方式显示，不自动重画；
- **独立阅读面板**：左侧分页列表可直达任意一页（页状态聚合：✓ 已画 / … 绘制中 / — 未画），
  **两种阅读模式一键切换**——「翻页」整页居中（←→ / 侧栏按钮翻页），「连续」同一批漫画纸竖向连滚
  （滚轮 / ↑↓ / PageUp/PageDown，Home/End 到首尾；滚到哪页侧栏/页码跟随高亮）；两种模式看的是同一套页面；
  选择会被记住，下次打开沿用；单格重绘、「继续生成」回到生成向导；一键导出**自包含 HTML 漫画**
  （同样的整页构图 + 连续/翻页模式切换 + 打印全页，内嵌图片，浏览器直接打开）；
- **一键成漫**：AI 按剧情规划整页脚本（目标 1~16 页），或每回合生成一页；本地逐回合模式没有原文台词时不编造引语。
  角色外观表、页内分镜与台词一并传给生图模型。使用竖版尺寸；横版设置交换长宽，正方形或未设置时使用 1024×1536。
  端点须支持该尺寸与图中文字；漫画整页不会沿用可能禁止文字的全局负面提示词。
- **起止自选**：默认从第 1 回合讲到当前进度，也可自选范围；续画只追加未覆盖回合的新页，保留已有图片。
- **断点续跑**：按整页串行绘制，暂停/继续、失败页重试；中断重启后跳过已完成页。

### 角色闪卡工坊

- **独立图鉴面板**（顶栏「作品」→ 角色闪卡，或 Ctrl+Shift+H）：卡片墙直接预览卡面（稀有度角标 + 角色名 + 日期），
  点卡面或「查看」开 3D 查看器，「生成新闪卡」从世界线选角——AI 规划卡面（两段式生图 + 自动抠图排版）；
- **3D 全息查看器**：可拖转、随视角流光、翻面查看；卡片落盘与角色档案挂钩，随时重开回味。
...
- 字体（无衬线/衬线）、圆角、密度、字号（Ctrl+=/-/0 或主题弹层磁贴）、阅读列宽全部可调、即时预览；阅读列右缘可拖拽自由调宽（持久化，双击复位），主题弹层显示自定义像素；
...
| 文件 | 说明 |
|---|---|
| `SixWorlds-Setup-1.5.3.exe` | 安装版：可选目录、创建快捷方式 |
| `SixWorlds-Portable-1.5.3.exe` | 免安装单文件，双击即用 |
| `SixWorlds-Android-1.5.3.apk` | Android 配套 App（导入桌面进度包接续游玩） |
...
| `Ctrl+Shift+M` | 漫画回放 | `Ctrl+Shift+H` | 角色闪卡 |
| `Ctrl+,` | 设置 | `Ctrl+/` | 快捷键面板 |
| `Ctrl+=` / `Ctrl+-` / `Ctrl+0` | 字号 缩放/缩小/复位 | `Esc` | 关闭浮层 / 清空搜索 |
| 拖拽阅读列右缘 | 自由调宽（`←`/`→` 微调，双击复位） | | |
...
最近一次仓库审查、修复说明与产品迭代建议见 [2026-09-21 审查报告](docs/repository-product-review-2026-09-21.md)。
新回合会保留最近 60 份行动前回溯点，供重生成与 IF 分支恢复对应记忆；旧消息无回溯点或回溯点过期时会提示并保留原剧情。
...
<details>
<summary><b>消息上出现「记忆记账中…」或「状态未落账」是什么？</b></summary>
「记忆记账中…」是正常现象：剧情已可阅读（阅读永不被记账阻塞），系统正在后台把这一回合写入世界记忆，完成自动消失。「状态未落账」则说明该回合的结构化状态没有正式提交：系统已自动重试并挂起待补录，点击消息上的徽标或顶部横幅可一键补录（会带上首次被拒的原因，成功率更高）；若某个模型频繁出现，建议换推理能力更强的模型。完整原因可在检查器的回合日志中查看。
</details>
