...
- **真·漫画页**（顶栏「作品」→ 漫画回放，或 Ctrl+Shift+M）：幕以 **4 格/页** 拼成漫画页——2 列分格网格、
  hero 跨双列大格开篇、宽扁格走转场、竖长格立对峙；**对白气泡叠在画格上**（说话人 + 台词 + 语气形态：
  喊话锯齿重气泡 / 内心独白云朵虚线泡 / 低语点线泡），旁白走格顶压暗条——台词是真实文本，不是生图画出来的乱码；
- **独立阅读面板**：左侧分页列表可直达任意一页（页状态聚合：✓ 已画 / … 绘制中 / — 未画），
  **两种阅读模式一键切换**——「翻页」单行本式一页一屏（←→ / 侧栏按钮翻页），「连续」条漫式整部
  连成长条往下滚动（滚到哪页侧栏/页码跟随高亮）；选择会被记住，下次打开沿用；单格重绘、「继续生成」
  回到生成向导；一键导出**自包含 HTML 漫画**（同样分格 + 气泡排版 + 连续/翻页模式切换，内嵌图片，浏览器直接打开）；
- **一键成漫**：引擎从全量剧情账本精选关键幕（AI 分镜师取舍，8~32 幕可调）或按回合逐幕展开，
  自动生成**角色外观表**（发色/服装/体型）逐幕注入，跨幕不变脸；每幕由分镜师标注**格子尺寸**（hero/square/wide/tall）
  与 **1~3 条台词**（说话人 + 台词 + 语气），画格只画「一个瞬间」且画面无文字（对白由气泡叠加）；
- **起止自选**：默认从第 1 回合讲到当前进度，也可自选起止回合；继续推进剧情后再点一键，**只补画新幕**（已有的幕保留）；
- **断点续跑**：逐幕串行绘制，随时暂停/继续；中断重启后自动差量补缺。
