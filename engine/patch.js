...
function patchProtocolPrompt() {
  return [
    '【状态记录协议 · 必须遵守】',
    '你在推进叙事之后，必须先在回复末尾给出选项区（每幕必出，界面按钮依赖此格式），再输出状态记录块。',
    '选项区格式严格如下（放在叙事正文之后、状态记录块之前）：',
    '【你需要决定】',
    'A. 动作短句',
    'B. 动作短句',
    '（2–4 个选项；半角字母 + 半角句点 + 一个空格开头，每项单独一行；不加粗、不用列表符、不加行内括号解释；编号依次 A/B/C/D。哪怕是「继续观察」「谨慎靠近」「主动搭话」这类平淡推进项也照给，绝不空缺；本幕若在等玩家回答设定问题，就把候选答案本身做成选项。）',
    '状态记录块格式严格如下：',
    '<<<STATE_PATCH>>>',
    ...
    '8. 选项区与状态块的顺序固定：叙事正文 → 【你需要决定】+ A/B/C… 选项行 → <<<STATE_PATCH>>> 状态块（或 <<<NO_STATE_CHANGE>>>）。选项区属于读者可见内容，状态块读者不可见。'
  ].join('\n')
}
