/* ======== 六面世界 · 极简桌面聊天窗 ========
 * 【维护策略】本目录（ui/classic/）＝「经典界面」方案，已冻结为维护态：
 *  只修阻断性 bug 与数据安全问题，不再新增功能；功能演进集中在 ui/proto/（原型工作台方案，
...
(() => {
  ...
  function activateSession(s) {
    if (busy || engineBusy) { toast('请等当前回合与记忆保存完成', 'info'); return }
...
const KData = window.KernelData.createKernelData({
  ...
  const Gallery = window.GalleryPanel.createGallery({
    ...
    renderSessionList, renderMessages, updateTitle,
    summarize,
    viewIllust, generateIllust, downloadIllust, illustsOf,
  ...
  function refreshWorksPop() {
    ...
    if (comicSub) {
      comicSub.textContent = (comic && comic.panels && comic.panels.length)
        ? ('已画 ' + comic.panels.filter((p) => p.illust || (Array.isArray(p.illusts) && p.illusts.length)).length + ' / ' + comic.panels.length + ' 幕')
        : '还没有分镜'
    }
