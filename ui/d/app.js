/* ======== 六面世界 · 方案D「通用阅读」界面（完整交互版） ========
 * 基线：ui/classic/（功能/数据/引擎桥全部沿用），叠加方案D 交互层——
 *  · 顶栏分段控制（故事 ⇄ 内核）沿用 classic area-switch，皮肤为胶囊分段
 *  · 左侧面板三态：停靠 dock（内联占位）/ 细轨 rail（56px 图标条）/ 悬停浮层 float（不占布局）
 *    —— 停靠与否仅由图钉控制；Ctrl+B / ☰ 只负责收起与浮层开合
 *  · 内核六步（00-05）排版与 classic kernel-design-v3 一致
 * 与 classic / proto 共享同一份 localStorage 与引擎数据；
 * 涉及共享数据结构的改动必须三侧同步。
 */
(() => {
  'use strict'
  ...
  // ---- 图像模型预设已随设置迁至独立窗口（ui/classic/settings.js） ----
...
const Rail = window.RailPanel.createRailPanel({ $, msgEl: () => document.getElementById('messages'), cancelHideAnim, hideWithAnim })
  ...
  // ============ 方案D 左侧面板三态：停靠 dock / 细轨 rail / 悬停浮层 float ============
  // 语义（与 UI效果图/方案D-完整交互版 一致）：
  //  · 停靠与否只由图钉控制（cfg.sbDock）；取消停靠后，面板只以浮层出现，永远不占布局
  //  · Ctrl+B / ☰：停靠态=收起为细轨；细轨态=开合浮层（toggleFloat）
  //  · 悬停细轨 150ms 浮出，移开 250ms 收回；浮层中操作完自动收回并闪示细轨
  const sidebarEl = $('sidebar')
  const railStripEl = $('rail-strip')
  const SB_MIN = 160, SB_MAX = 420
  const narrowMQ = window.matchMedia('(max-width: 760px)')

  // 兼容旧配置：sidebarCollapsed=true 迁移为未停靠；新配置键 sbDock（默认 true）
  let sbDock = cfg.sbDock === undefined ? !cfg.sidebarCollapsed : !!cfg.sbDock
  let sbPeeking = false      // 未停靠时的浮层展开态
  let sbPeekTimer = null, sbCloseTimer = null

  function sbCollapsed() { return !sbDock || narrowMQ.matches }  // 窄窗强制细轨（同 classic 精神）

  function applySidebar() {
    const rail = sbCollapsed()
    const floating = rail && sbPeeking   // 细轨态 + 浮出 = 浮层（不占布局、完整面板）
    document.body.dataset.sb = rail ? 'rail' : 'dock'
    // collapsed 类：仅"细轨且未浮出"时挂（隐藏 48px 窄条视觉；浮层态要完整面板，绝不带 collapsed）
    sidebarEl.classList.toggle('collapsed', rail && !sbPeeking)
    sidebarEl.classList.toggle('sb-float', floating)
    document.body.classList.toggle('sb-collapsed', rail && !sbPeeking)
    if (!rail) sidebarEl.style.width = Math.min(SB_MAX, Math.max(SB_MIN, Number(cfg.sidebarWidth) || 200)) + 'px'
    else sidebarEl.style.width = ''
    railStripEl.classList.toggle('show', rail)
    ...
    if (tog) {
      ...
      tog.title = rail ? '浮层展开面板（Ctrl+B）' : '收起为细轨（Ctrl+B）'
    }
    const pin = $('btn-sb-pin')
    if (pin) {
      pin.classList.toggle('on', sbDock)
  ...
  // 图钉：唯一能改变停靠状态的入口
  let sbRailArmedAt = 0   // 细轨刚出现的时刻：抑制期内忽略 mouseenter，防止"取消停靠瞬间指针恰在细轨上"立刻浮出
  function sbSetDock(dock, { silent } = {}) {
    sbDock = !!dock
    clearTimeout(sbPeekTimer)
    clearTimeout(sbCloseTimer)
    if (!sbDock) {
      sbPeeking = false
      sbRailArmedAt = Date.now()
    }
    cfg.sbDock = sbDock
    cfg.sidebarCollapsed = !sbDock   // 同步旧键，切回 classic/proto 时语义正确
    saveStore()
    applySidebar()
    if (!silent) toast(sbDock ? '已停靠：面板占位常驻' : '已取消停靠：悬停细轨即浮层', 'info', 2200)
  }

  function toggleSidebar() {
    // 方案D 语义：停靠态→收起细轨；细轨态→开合浮层（不回到停靠）
    if (sbDock) sbSetDock(false)
    else { sbPeeking = !sbPeeking; sbRenderFloat() }
  }

  // —— 细轨悬停浮出 / 移开收回（150/250ms；细轨出现后 500ms 抑制期防误浮）——
  railStripEl.addEventListener('mouseenter', () => {
    if (sbDock) return
    if (Date.now() - sbRailArmedAt < 500) return   // 刚取消停靠：指针可能恰在细轨上，不立即浮出
    clearTimeout(sbCloseTimer)
    sbPeekTimer = setTimeout(() => { sbPeeking = true; sbRenderFloat() }, 150)
  })
  railStripEl.addEventListener('mouseleave', () => {
    clearTimeout(sbPeekTimer)
    sbCloseTimer = setTimeout(() => { if (sbPeeking && !sidebarEl.matches(':hover')) { sbPeeking = false; sbRenderFloat() } }, 250)
  })
  sidebarEl.addEventListener('mouseleave', () => {
    if (sbDock) return
    clearTimeout(sbCloseTimer)
    // 250ms 后收回，但若此刻浮层重新打开且指针已在浮层/细轨上（如点击图钉取消停靠后马上 Ctrl+B），不收回
    sbCloseTimer = setTimeout(() => {
      const overSb = sidebarEl.matches(':hover')
      const overRail = railStripEl.matches(':hover')
      if (sbPeeking && !overSb && !overRail) { sbPeeking = false; sbRenderFloat() }
    }, 250)
  })
  sidebarEl.addEventListener('mouseenter', () => clearTimeout(sbCloseTimer))
  function sbClosePeek() { if (sbPeeking) { sbPeeking = false; sbRenderFloat() } }

  // —— 细轨图标：世界线/搜索=浮层展开（搜索聚焦）；画廊/内核/设置=直达目标并收浮层 ——
  $('r-lines').addEventListener('click', () => { if (!sbDock) { sbPeeking = true; sbRenderFloat() } })
  $('r-search').addEventListener('click', () => {
    if (!sbDock) { sbPeeking = true; sbRenderFloat(); setTimeout(() => { const s = $('sb-search'); if (s) s.focus() }, 250) }
  })
  $('r-gallery').addEventListener('click', () => { openGallery(); sbClosePeek() })
  $('r-kernel').addEventListener('click', () => { sbClosePeek(); openKernelHub() })
  $('r-settings').addEventListener('click', () => { toggleThemePop() })

  // —— 图钉按钮：唯一改变停靠状态的入口 ——
  $('btn-sb-pin').addEventListener('click', () => sbSetDock(!sbDock))

  // —— 浮层中完成操作后自动收回（点会话项）——
  const railFlash = () => {
    const f = railStripEl.querySelector('.rb')
    if (f) { f.classList.add('flash'); setTimeout(() => f.classList.remove('flash'), 600) }
  }
  const sbAutoRetract = () => { if (!sbDock && sbPeeking) { sbPeeking = false; sbRenderFloat(); railFlash() } }
  document.getElementById('session-list').addEventListener('click', (e) => {
    if (sbDock) return
    const item = e.target.closest('.session-item, .session-del')
    if (item) setTimeout(sbAutoRetract, 120)
  })

  // 拖拽把手（右缘 7px 热区，hover 显示竖线）——仅停靠态有意义
  const sbHandle = document.createElement('div')
  sbHandle.className = 'sidebar-handle'
  sbHandle.title = '拖动调整宽度 · 拖到最窄自动收为细轨'
  sidebarEl.appendChild(sbHandle)

  let sbDragging = false, sbStartX = 0, sbStartW = 0
  sbHandle.addEventListener('mousedown', (e) => {
    if (sbDock && !narrowMQ.matches) {
      sbDragging = true
      sbStartX = e.clientX
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    sidebarEl.style.width = Math.min(SB_MAX, Math.max(SB_MIN, w)) + 'px'
  })
  window.addEventListener('mouseup', () => {
    ...
    cfg.sidebarWidth = Math.round(sidebarEl.getBoundingClientRect().width)
    saveStore()
  })
  // 双击把手：收起为细轨
  sbHandle.addEventListener('dblclick', () => sbSetDock(false))

  // 窗口跨过窄屏断点时自动应用（窄→强制细轨，宽→恢复停靠设置）
...
const KData = window.KernelData.createKernelData({
  ...
  // ---- 方案D 侧栏停靠（data-sb）：停靠展开 / 细轨浮层（与图钉同一状态机） ----
  themePopEl.querySelectorAll('[data-sb]').forEach((tile) => {
    tile.addEventListener('click', (e) => {
      e.stopPropagation()
      sbSetDock(tile.dataset.sb === 'dock', { silent: true })
      syncSbTiles()
    })
  })
  function syncSbTiles() {
    themePopEl.querySelectorAll('[data-sb]').forEach((t) => {
      const on = (t.dataset.sb === 'dock') === sbDock
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') {
      ...
      else if (!sbDock && sbPeeking) sbClosePeek()   // 方案D：浮层面板逐层最外
